...
(function($, document) {
	...
	$.fn.validate = function(tips){
		...
		var content;
		if($(this).attr("class") == "datebox-f combo-f"){
			content = $(this).datebox('getValue'); //日期类型
		}else{
			content = $(this).val();
		}
		if(content == "" || $.trim(content).length == 0){
        	$.messager.alert("消息", tips + "不能为空！","warning");
        	throw SyntaxError();
        }
	}
	...
	$.fn.searchForm = function(options) {
		...
		this.reSet = function() {
			...
			$('.easyui-combotree').combotree('setValue', '--请选择--'); 
			return;
		}
		// 执行查询功能
		...
		$('tr', searchFormVar).each(function(index, obj) {
			...
			// class属性为interlace的行背景为白色
			if(typeof($(obj).attr("class")) != 'undefined' && $(obj).attr('class').indexOf('interlace') > -1){
		...
		function submit_do(url, func) {
			...
			if (comiitHandleFun) {
				...
				jQuery.post(url,
						...
						if(flag.pageset.message != ""){
							$.messager.alert(MESSAGE_PLAT.ERROR, flag.pageset.message, 'error');
						}else{
			...
			if (_pageType == _viewType) {
				...
				$(':text', form).each(
						function(index, value) {
							...
							$(value).after(
									'<div style="white-space: normal;">' + obj.replace("00:00:00","") + '</div>');
							$(value).remove();
						});
				$('textarea', form).each(
						function(index, value) {
...
$(function(){
	...
	var height = document.body.scrollHeight - 30 + "px;"; //网页高度
	if($(".scrollbar").attr("style") == undefined){
		$(".scrollbar").attr("style","height: "+height+"overflow-y :auto");
	}else{
		$(".scrollbar").attr("style",$(".scrollbar").attr("style") + "height: "+height+"overflow-y :auto");
	}
