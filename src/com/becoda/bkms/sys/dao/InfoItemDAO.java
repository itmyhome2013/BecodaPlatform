...
/**
 * Created by IntelliJ IDEA. User: kangdw Date: 2015-3-4 Time: 11:22:55 To
...
public class InfoItemDAO extends GenericDAO {
	 ...
	 * @return boolean
	 */
	public boolean checkInfoItemID(String setId, String itemId)
	...
	/**
	 * 添加信息项 县添加信息项信息 然后再将字段添加到相应数据表中
	 * 
	 * @param infoItem
	 *            信息项BO
	 */
	public void createInfoItem(InfoItemBO infoItem)
			throws RollbackableException {
		try {
			if ("".equals(infoItem.getItemDataLength())
					|| infoItem.getItemDataLength() == null)
				infoItem.setItemDataLength("100");
			hibernateTemplate.save(infoItem);

			StringBuffer sb = new StringBuffer();
			sb.append("alter table ").append(infoItem.getSetId()).append(
					" add ( ");
			String field = infoItem.getItemId();
			int length;
			if (SysConstants.INFO_ITEM_DATA_TYPE_REMARK.equals(infoItem
					.getItemDataType())) // 如果是备注型，则长度存储4000
				length = 4000;
	 ...
	 *            信息项ID
	 */
	private void deleteInfoItem(String setId, String itemId)
			throws RollbackableException {
		try {
			...
			InfoItemBO p = (InfoItemBO) hibernateTemplate.find(sf.toString())
					.get(0);
			hibernateTemplate.delete(p);
			// 删除数据表字段
			sf = new StringBuffer();
			sf.append("alter table ").append(setId).append(" drop ( ").append(
					itemId).append(")");
	 ...
	 *            状态 1 启用 0 禁用
	 */
	public void setStatus(String setId, String itemId, String status)
			throws RollbackableException {
		try {
			StringBuffer sb = new StringBuffer();
			sb.append("from InfoItemBO iItem where iItem.setId='");
			...
			sb.append("' and iItem.itemId = '");
			sb.append(itemId).append("'");
			InfoItemBO iItem = (InfoItemBO) hibernateTemplate.find(
					sb.toString()).get(0);
	 ...
	 * @param sType
	 * @param status
	 *            过滤标记 status =null or "" 则查询所有,否则查启用的指标集和项
	 * @return
	 * @throws RollbackableException
	 */
	public InfoItemBO[] queryInfoItems(String sType, String status)
			throws RollbackableException {
		try {
			StringBuffer sql = new StringBuffer();
			sql
					.append(
							"select p from InfoSetBO s,InfoItemBO p where s.setId=p.setId and p.editProp<>'")
					.append(SysConstants.INFO_ITEM_EDIT_PROP_HIDE).append("'");
			if (sType != null && !"".equals(sType))
				sql.append(" and s.set_sType='").append(sType).append("'");
			if (status != null && !"".equals(status))
				sql.append(" and s.setStatus ='").append(
						SysConstants.INFO_STATUS_OPEN).append("'").append(
						" and p.itemStatus='").append(
						SysConstants.INFO_STATUS_OPEN).append("'");
			...
			if (list == null || list.size() == 0)
				...
				return (InfoItemBO[]) list.toArray(new InfoItemBO[0]);
		} catch (Exception e) {
	 ...
	 * @throws RollbackableException
	 */
	public InfoItemBO[] queryInfoItems(String setId, String status,
			String dataType, String property, boolean getCtl)
			throws RollbackableException {
		try {
			StringBuffer sql = new StringBuffer();
			...
			if (!getCtl)
				tmp.append(" and p.editProp<>'").append(
						SysConstants.INFO_ITEM_EDIT_PROP_HIDE).append("'");
	 ...
	 * @throws RollbackableException
	 */
	public void issueInfoItem(String setId, String itemId)
			throws RollbackableException {
		try {
			StringBuffer sb = new StringBuffer();
			sb.append("from InfoItemBO iItem where iItem.setId='");
			...
			sb.append("' and iItem.itemId = '");
			sb.append(itemId).append("'");
			InfoItemBO p = (InfoItemBO) hibernateTemplate.find(sb.toString())
					.get(0);
			p.setItemIssue("1");
		} catch (Exception e) {
	...
	public String getNewItemId(String setId, String property)
		...
		try {
			...
			List list = hibernateTemplate
					.find("select b.itemId from InfoItemBO b where b.setId ='"
							+ setId + "'");
			if (list != null && list.size() > 0) {
				for (int i = ini; i <= end; i++) {
					newItemId = "00000" + Integer.toString(i);
					newItemId = setId
							+ newItemId.substring(newItemId.length() - 3);
