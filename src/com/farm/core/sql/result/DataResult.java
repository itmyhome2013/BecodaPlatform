...
public class DataResult {
	 ...
	 * @return 结果集合封装对象
	 */
	public static DataResult getInstance(List<Map<String, Object>> data,
			int _totalSize, int _currentPage, int _PageSize) {
		...
		try {
			for (Map<String, Object> node : list) {
				...
				if (node.get(title) instanceof java.sql.Date) {
					java.util.Date d = new java.util.Date(((java.sql.Date) node
							.get(title)).getTime());
	...
	public List<Object> getObjectList(Class class1) {
		...
		for (Map<String, Object> node : getResultList()) {
			...
			try {
				...
				for (Entry<String, Object> field : node.entrySet()) {
					BeanUtils.setProperty(obj, field.getKey().toLowerCase(),
							field.getValue());
				}
			} catch (Exception e) {
